...
**Model (Sections 3-4).** We formalize verification reversal as a cost inequality and derive a sequential forwarding game. *Claim 1*: When verification costs exceed production costs for a substantial artifact class, cascade equilibria emerge in which rational agents propagate unverified artifacts regardless of private beliefs.

**Productivity Measurement (Sections 5-7).** We distinguish measured from verification-adjusted productivity and formalize epistemic debt as a stock variable. *Claim 2*: Under verification reversal, measured TFP can rise while utility-relevant productivity stagnates (synthetic productivity). *Claim 3*: Model-generated content contaminates measurement substrates, extending recognition lags.

**Self-Correction Failure (Section 8).** We analyze recursive verification and market selection. *Claim 4*: Both mechanisms fail structurally because the cascade equilibrium is self-reinforcing on the signals that would guide correction.
...
> **Definition (Verification Reversal).** The regime in which the marginal cost of producing an artifact is lower than the marginal cost of verifying it, formally: ∂C_prod/∂Y < ∂C_verify/∂Y.
...
Empirical studies document productivity gains from generative AI across knowledge-work tasks: coding, writing, translation, analysis. The gains are real within the measurement frame employed. But the measurement frame matters.
...
- **Verify**: Incur cost C_verify to check the artifact against independent criteria.
- **Forward**: Pass it on with minimal scrutiny at cost C_forward, where C_forward << C_verify.

Let p_i(valid) denote agent i’s private belief that the artifact is valid. Let p_n(cascade) denote the probability, inferred from observing upstream behavior, that the prevailing cascade is correct.

> **Claim 1 (Forwarding Condition).** The agent forwards when:
> 
> C_verify - C_forward > B · (p_i(valid) - p_n(cascade))
> 
> where B is the expected benefit of catching an error.

When the cost gap is large, this condition holds even when private beliefs are skeptical. The agent thinks something might be wrong. The agent also thinks: surely someone upstream would have caught it. And: if I’m the one who stops the line, I bear the cost of delay. And: my skepticism is just one signal against the revealed behavior of everyone before me.

So the agent forwards. And the next agent sees one more forward in the sequence, updates p_n(cascade) upward, and forwards in turn. The cascade deepens with each step.

### 4.1 Information Blockage
...
> **Definition (Information Blockage).** A state of high throughput, high confidence intervals on dashboards, and no independent signal about whether any of it corresponds to reality.
...
> **Claim 2 (Synthetic Productivity).** Measured TFP rises because artifact volume increases. Verification-adjusted, utility-relevant productivity stagnates or declines. The productivity miracle is an accounting error waiting for an audit.
...
> **Claim 3 (Recognition Lag Extension).**
> 
> dτ/dα > 0
> 
...
> **Claim 4 (Selection Failure).** The organizations best positioned to weather a forcing event do not survive to the forcing event. High-verification strategies are systematically selected out during the accumulation phase.
...
The computational substrate for AI-mediated production imposes hard physical limits. Global data center power demand is projected to more than double by 2030, reaching 8% of total US electricity consumption, driven by intensive computational requirements of large language models (Goldman Sachs, 2024). This creates a binding constraint: measured productivity gains from artifact generation are partially offset by rising energy costs and infrastructure requirements. The wedge between digital throughput and physical resource consumption suggests that synthetic productivity, even when measured correctly, may face diminishing returns as energy efficiency gains decelerate and computational demands scale super-linearly with model capabilities.
...
1. A sequential forwarding model for cascade formation under high verification costs, showing how rational individual behavior aggregates to collective informational blockage.
1. A formal separation of measured from verification-adjusted productivity, with conditions under which the wedge grows over time.
1. An endogeneity share parameter capturing measurement contamination from model-generated content.
1. Structural analysis of why “AI verifies AI” lacks independent rejection signals and why market selection operates on lagging indicators that reward low-verification strategies.
...
GitClear (2024). *Coding on Copilot: 2024 Data Suggests Downward Pressure on Code Quality*. Technical Report. Retrieved January 2025 from https://www.gitclear.com/coding_on_copilot_data_shows_ais_downward_pressure_on_code_quality

Goldman Sachs (2024). *AI is poised to drive 160% increase in data center power demand*. Goldman Sachs Global Investment Research.
